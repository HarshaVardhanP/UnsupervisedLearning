...
for init = 1:2
    ...
    if i==1
        ...
        AE = load('Models/model_autoencoder.mat');
        AE_model = AE.model;
        AE_W = AE_model.weights{1};
        model.weights{1}=RBM_W;
    end
    if i==2
        %---Model Initialization with Weights----%
        DAE = load('Models/model_DenoisyAutoencoder.mat');
        DAE_model = DAE.model;
        DAE_W = DAE_model.weights{1};
        model.weights{1}=DAE_W;
    end
    %---------Train the model----------%
...
legend('Train - AutoEncoder Pretrained','Train - DenoisyAutoEncoder Pretrained','Location','northwest')
title('Training Classification Accuracy')

subplot(1,2,2),plot(valid_Cerr(:,1)), hold on
subplot(1,2,2),plot(valid_Cerr(:,2)) 
legend('Valid - AutoEncoder Pretrained','Valid - DenoisyAutoEncoder Pretrained','Location','northwest')
